...
const SignIn = (props) => {
  ...
  const showError = () => {
    if (hasError) {
      return (
        <div style={{color: "red", fontSize: "16px", paddingBottom: "20px"}}> {sysmessage}</div>
      )
    } else if (data.status) {
      return (
      ...
      return (
        <div style={{color: "red", fontSize: "16px", paddingBottom: "20px"}}>{data.error}</div>
      )
    }
  };
  ...
  return (
    <Fragment>
      ...
      >
        <div className={classes.BlankCanvas} style={{height: "370px"}}>
