export const loadEventInfo = (eventTix) => {
    ...
    "eventType" in eventTix ? eventDescription.eventType = eventTix.eventType : eventDescription.eventType = "live"
    "refundPolicy" in eventTix ? eventDescription.refundPolicy = eventTix.refundPolicy : eventDescription.refundPolicy = "noRefunds"
    ...
    if ("startDateTime" in eventTix) {
        ...
        eventDescription.startDate.setMinutes( eventDescription.startDate.getMinutes() + eventDescription.startDate.getTimezoneOffset() );
    ...
    if ("endDateTime" in eventTix) {
        ...
        eventDescription.endDate.setMinutes( eventDescription.endDate.getMinutes() + eventDescription.endDate.getTimezoneOffset() );
    ...
    if ("tickets" in eventTix && eventTix.tickets.length !== 0) {
      ...
      eventTix.tickets.forEach((tix, index) => {
        ...
        if ("priceFunction" in tix && "form" in tix.priceFunction && "args" in tix.priceFunction) {
          tempPriceFeature = tix.priceFunction.form;

          if (tempPriceFeature === "promo" && "promocodes" in tix.priceFunction.args) {
...
/*
    ...
    setOriginalEventDescription(eventDescription);
*/
  };